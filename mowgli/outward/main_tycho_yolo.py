...
def main(input_dir, mode, model_name, output_dir):
    ...
    for filename in os.listdir(input_dir):  
        if filename.endswith(".jpg") or filename.endswith(".png"):   
            ...
            img_resized, dw, dh = resize_and_pad(img, (384, 640))
            ...
            #random_image = np.random.rand(384, 640, 3) * 255                                                                                                                                          
            ...
            boxes_pred = infer_request.get_output_tensor(0)
            scores_pred = infer_request.get_output_tensor(1)
            print(f"shape of boxes_pred for filename : {filename} is {boxes_pred.shape}")
            print(f"shape of scores_pred for filename : {filename} is {scores_pred.shape}")

            # Write output boxes as numpy raw file
            boxespredpath = os.path.join(output_dir, f"{filename.split('.')[0]}_boxes.raw")
            boxes_pred.data.tofile(boxespredpath)

            # Write output scores as numpy raw file
            scorespredpath = os.path.join(output_dir, f"{filename.split('.')[0]}_scores.raw")
            scores_pred.data.tofile(scorespredpath)
